...
feature -- More Attributes
	set_enum_list : ARRAYED_LIST[STRING]

feature -- evaluator visitors for each expression
	visit_addition(e: BINARY_OPERATION)
	local
		i : INTEGER
		left_visit_evaluate : EVALUATOR
		right_visit_evaluate : EVALUATOR
	do
		create set_enum_list.make (0)
	...
	local
		visit_evaluate_left : EVALUATOR
		visit_evaluate_right : EVALUATOR
		--symbol : TERMINALS
	do
		create set_enum_list.make (0)
	...
	do
		...
		e.left.accept (visit_evaluate_left)
		e.right.accept (visit_evaluate_right)
		-- Set the starter list to the first list
...
feature{NONE}-- other class functions
